...
final class DiffVersionsHandler
{
    public function __invoke(int $fromId, int $toId): DiffVersionsResult
    {
        ...
        if (!$object1->isAllowed('versions') || !$object2->isAllowed('versions')) {
            throw new AccessDeniedHttpException('Permission denied for version ids [' . $fromId . ', ' . $toId . ']');
        }
