...
final class DiffVersionsHandler
{
    ...
    public function __invoke(int $fromId, int $toId, string $requestSchemeAndHost): DiffVersionsResult
    {
        if (!HtmlToImage::isSupported() || !class_exists('Imagick')) {
            return new DiffVersionsResult(supported: false);
        }

        $versionFrom = Version::getById($fromId);
        $docFrom = $versionFrom?->loadData();

        if (!$docFrom instanceof Document) {
            throw new DocumentNotFoundException($fromId);
        }

        $versionTo = Version::getById($toId);
        $docTo = $versionTo?->loadData();

        if (!$docTo instanceof Document) {
            throw new DocumentNotFoundException($toId);
        }
        ...
        $prefix = Config::getSystemConfiguration('documents')['preview_url_prefix'] ?: $requestSchemeAndHost;
