...
final class AddDocumentHandler
{
    ...
    public function __invoke(
        ...
        string $key,
        ?string $docTypeId,
        ?string $translationsBaseDocumentId,
        ?string $language,
        ...
        $parentDocument = Document::getById($parentId);
        ...
        $intendedPath = $parentDocument->getRealFullPath() . '/' . $key;
        ...
        } elseif (in_array($type, ['page', 'snippet', 'email'])) {
            $createValues['controller'] = $this->defaultDocumentController;
        }

        if ($inheritanceSource !== null) {
            $createValues['contentMainDocumentId'] = $inheritanceSource;
        }

        $document = match ($type) {
            'page' => $this->createPage($parentDocument, $createValues, $title, $name),
            ...
            default => $this->createCustomType($type, $parentDocument, $createValues),
        };

        // link translation if translationsBaseDocument given
        if ($translationsBaseDocumentId !== null) {
            $translationsBaseDocument = Document::getById((int) $translationsBaseDocumentId);
