...
final class TreeGetDocumentChildrenHandler
{
    ...
    public function __invoke(array $allParams): TreeGetDocumentChildrenResult
    {
        $paginated = isset($allParams['limit']);
        $limit = (int)($allParams['limit'] ?? 100000000);
        $offset = (int)($allParams['start'] ?? 0);

        $filter = $allParams['filter'] ?? null;
        if (!is_null($filter)) {
