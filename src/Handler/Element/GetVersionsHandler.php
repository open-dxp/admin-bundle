...
final class GetVersionsHandler
{
    ...
    public function __invoke(int $id, ?string $type): GetVersionsResult
    {
        ...
        if (!$element) {
            throw new NotFoundHttpException($type . ' with id [' . $id . "] doesn't exist");
        }

        if (!$element->isAllowed('versions')) {
            throw new AccessDeniedHttpException('Permission denied, ' . $type . ' id [' . $id . ']');
        }
