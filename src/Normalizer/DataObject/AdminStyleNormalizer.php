...
final class AdminStyleNormalizer implements ElementResponseNormalizerInterface
{
    ...
    public function normalize(ElementInterface $element, array &$data, array $context = []): void
    {
        ...
        $data['general']['iconCls'] = $adminStyle->getElementIconClass() !== false ? $adminStyle->getElementIconClass() : null;
        if (!$data['general']['iconCls']) {
            $data['general']['icon'] = $adminStyle->getElementIcon() !== false ? $adminStyle->getElementIcon() : null;
        } else {
            $data['general']['icon'] = null;
        }
        if ($adminStyle->getElementCssClass() !== false) {
            if (!isset($data['general']['cls'])) {
                $data['general']['cls'] = '';
            }
            $data['general']['cls'] .= $adminStyle->getElementCssClass() . ' ';
        }
        $data['general']['qtipCfg'] = $adminStyle->getElementQtipConfig();
        $elementText = $adminStyle->getElementText();
        if ($elementText !== null) {
            $data['general']['text'] = $elementText;
        }
    }
}
